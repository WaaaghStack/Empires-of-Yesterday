...
var _ghost_path_lines: Array[Line2D] = []
var _ghost_preview_room: Room = null
...
func handle_map_right_click(world_pos: Vector2) -> void:
	...
	if room:
		if not room.is_revealed:
			...
			return
		if is_paused and game_active:
			_update_ghost_preview(room)
...
func _spawn_squad() -> void:
	...
	if active_units.size() > 0:
		_select_squad("alpha")
	squad_roster.bind_units(active_units)
...
func _spawn_enemies() -> void:
	...
	for room in rooms:
		...
		var is_elite: bool = bool(room.get_meta("elite_slot", false))
		for i in range(count):
			var is_elite_slot: bool = is_elite and i == count - 1
			var arch: Enemy.Archetype = Enemy.pick_archetype_for_op(op_depth, _enemy_spawn_rng, is_elite_slot)
...
func _spawn_hives() -> void:
	...
	for room in rooms:
		if not room.get_meta("hive_room", false):
			continue
		var hive = HiveScene.instantiate()
		hive.setup(room, room.map_room_id, map_data.map_seed, map_data.enemy_stat_scale, RunState.op_index if RunState.run_active else 1)
	...
	if _hives_total > 0:
		log_message(
			"BIO SCAN: %d hive signature(s) on deck. Destroy or endure escalating waves." % _hives_total,
			GameTheme.ACCENT_WARN.to_html(),
			"alert",
		)


func register_spawned_enemy(enemy: EnemyUnit, room: Room) -> void:
	enemy.died.connect(_on_enemy_died)
	enemy.combat_hit.connect(_on_combat_hit)
	enemy.alarm_triggered.connect(_on_enemy_alarm_triggered)
	world.add_child(enemy)
...
func _on_hive_destroyed(hive) -> void:
	...
	if hive and hive.home_room:
		log_message("Hive neutralized in %s." % hive.home_room.room_name, GameTheme.ACCENT_SUCCESS.to_html(), "objective")
...
func _process(delta: float) -> void:
	...
	if _any_unit_extracting():
		_update_objectives()
	if is_paused and game_active and not mission_complete:
		var world_pos := _container_mouse_to_world()
		_update_ghost_preview(_get_room_at_position(world_pos))
	elif not _ghost_path_lines.is_empty():
...
func _on_soldier_entered_room(room: Room, _soldier: SoldierUnit) -> void:
...
func _refresh_comms_display() -> void:
	...
	for entry in _comms_entries:
		...
		if _comms_filter == "all" or _comms_filter == cat or (cat == "alert" and _comms_filter == "alerts"):
...
func _follow_squad(squad_id: String, allow_toggle: bool = true) -> void:
	...
	RunLog.info("Camera follow %s" % squad_id)
	if allow_toggle:
...
func _clear_camera_follow() -> void:
	...
	_update_camera_follow_buttons()
	RunLog.info("Camera follow cleared (manual pan)")
...
func _issue_order_to_selected(room: Room) -> void:
	...
	for unit in units:
		...
		log_message("%s → %s: %s" % [unit.soldier_name, OrderTypeLib.get_label(order), room.room_name], "white")
...
func _update_objectives() -> void:
	...
	if extraction_status_label:
		...
		if _objectives_cleared():
			var extraction := _get_extraction_room()
			if extraction and extraction.is_revealed:
				var alive := _alive_units()
				var extracted_count := alive.filter(func(u): return u.is_extracted).size()
				if alive.size() > 0 and extracted_count >= alive.size():
...
func _get_extraction_status_text() -> String:
	...
	var extraction := _get_extraction_room()
	if not extraction or not extraction.is_revealed:
		return "Evac: Zone unknown — explore to locate"
	var alive := _alive_units()
	if alive.is_empty():
		return "Evac: %s — no operators remaining" % extraction.room_name
	var extracted_count := alive.filter(func(u): return u.is_extracted).size()
	if extracted_count >= alive.size():
		return "Evac: %s — all operators extracted" % extraction.room_name
	var extracting_count := alive.filter(func(u): return u.is_extracting).size()
	var status := "Evac: %s — %d / %d extracted" % [extraction.room_name, extracted_count, alive.size()]
...
func _update_hud() -> void:
	...
	pause_label.text = "⏸ PAUSED — SPACE to resume" if is_paused else "▶ COMBAT PHASE — SPACE to plan"
	...
	if show_commander:
		...
		unit_order_label.text = "Order: %s | Default: %s" % [
			OrderTypeLib.get_label(unit.current_order),
...
func _update_fog_of_war() -> void:
	...
	for soldier in soldiers:
		for room in rooms_to_check:
			if room.is_extraction_room and _evac_search_gate > 0 and _searched_room_count < _evac_search_gate:
				continue
			if room.contains_local_point(soldier.position, 0.0):
				_reveal_and_search(room)
				entity_index.mark_revealed(room)
			elif fog_scale >= 1.0 and LineOfSightLib.can_reveal_room(soldier.position, room, rooms, door_nodes):
				room.reveal()
				if room.is_extraction_room:
	...
	for enemy in active_enemies:
		if not enemy.is_alive:
			continue
		var was_visible := enemy.is_visible_to_player
		var seen := false
...
func _reveal_and_search(room: Room) -> void:
	room.reveal()
...
func _apply_camera_zoom(zoom_in: bool) -> void:
	...
	if map_camera.zoom != before:
		RunLog.info("Camera zoom %.0f%%" % (_get_zoom_percent()))
	_clamp_camera_position()
	_update_zoom_indicator()
...
func _show_tutorial_hints() -> void:
	...
	if not TutorialStateLib.has_seen(TutorialStateLib.FLAG_PAUSE):
		log_message("Tip: SPACE toggles pause — plan orders while paused.", GameTheme.TEXT_MUTED.to_html())
		TutorialStateLib.mark_seen(TutorialStateLib.FLAG_PAUSE)
...
func _spawn_hold_wave() -> void:
	...
	for i in range(wave_size):
		var arch: Enemy.Archetype = Enemy.pick_archetype_for_op(op_depth, _enemy_spawn_rng, false)
...
func _order_supports_ghost_preview() -> bool:
	return pending_order in [
		OrderTypeLib.Type.MOVE,
...
func _clear_ghost_preview() -> void:
	_clear_ghost_path_lines()
	_clear_order_target_highlight()
	_ghost_preview_room = null


func _clear_ghost_path_lines() -> void:
	for line in _ghost_path_lines:
		line.queue_free()
	_ghost_path_lines.clear()
...
func _update_ghost_preview(target_room: Room) -> void:
	if not is_paused or not game_active or mission_complete or spawn_selection_active:
		_clear_ghost_preview()
		return
	if _order_supports_target_highlight() and target_room and target_room.is_revealed:
		_update_order_target_highlight(target_room)
	else:
		_clear_order_target_highlight()
	if not _order_supports_ghost_preview():
		_clear_ghost_path_lines()
		_ghost_preview_room = null
		return
	if not target_room or not target_room.is_revealed:
		_clear_ghost_path_lines()
		_ghost_preview_room = null
		return
	if target_room == _ghost_preview_room and not _ghost_path_lines.is_empty():
		return
	_clear_ghost_path_lines()
	_ghost_preview_room = target_room
	var units := _get_selected_units()
	if units.is_empty():
		return
	var preview_order := pending_order
	if target_room.is_extraction_room and _objectives_cleared():
		preview_order = OrderTypeLib.Type.EXTRACT
	elif preview_order == OrderTypeLib.Type.EXTRACT and not target_room.is_extraction_room:
		return
	for unit in units:
		if unit.is_extracted:
			continue
		var line := Line2D.new()
		line.width = 2.5
		line.default_color = Color(0.35, 0.82, 1.0, 0.6)
		line.z_index = 25
		world.add_child(line)
		var from_room := _room_containing_unit(unit)
		var blocked: Array[String] = []
		var points: PackedVector2Array = path_graph.find_path(unit.position, target_room.position, blocked, from_room, target_room)
		var packed := PackedVector2Array([unit.position])
		for point in points:
			packed.append(point)
		if packed.size() < 2:
			packed.append(target_room.position)
		line.points = packed
		_ghost_path_lines.append(line)
...
func _setup_commander_ui() -> void:
	...
	for filter_id in ["all", "alerts", "combat", "objective"]:
		var btn := Button.new()
		btn.text = filter_id.capitalize()
		btn.toggle_mode = true
		btn.button_pressed = filter_id == "all"
		btn.pressed.connect(_set_comms_filter.bind(filter_id, btn))
		comms_filter_row.add_child(btn)
	left_vbox.add_child(comms_filter_row)
	left_vbox.move_child(comms_filter_row, 1)
	...
	hud_vbox.add_child(camera_follow_row)
	hud_vbox.move_child(camera_follow_row, hud_vbox.get_child_count() - 1)
